...
static void G_Suck( gentity_t *ent ) 
{
	gentity_t *target;
	...
	mins[0] = -ent->radius * 1.42;
	mins[1] = -ent->radius * 1.42;
	mins[2] = -ent->radius * 1.42;
	maxs[0] = ent->radius * 1.42;
	maxs[1] = ent->radius * 1.42;
	maxs[2] = ent->radius * 1.42;

	VectorAdd( ent->r.currentOrigin, mins, mins );
	VectorAdd( ent->r.currentOrigin, maxs, maxs );

	// get entities near the vortex
	...
	for( num -= 1 ; num > 0 ; num-- )
	{
		...
		if( Distance(ent->r.currentOrigin, target->r.currentOrigin) > ent->radius )
			continue;
...
{
	G_SpawnInt("radius", "500", &ent->radius );
	G_SpawnInt("force", "800", &ent->force );
	ent->think = G_Suck;
	ent->nextthink = level.time + FRAMETIME;
}